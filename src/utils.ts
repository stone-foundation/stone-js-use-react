...
export const buildLayoutComponent = async (
  ...
  const metavalue = container
    ...
    .get<MetaComponentEventHandler<ReactIncomingEvent>>(`stone.useReact.layout.${String(layoutName ?? 'default')}`)
...
export const buildAdapterErrorComponent = async (
  ...
  const metavalue = blueprint.get<MetaComponentEventHandler<ReactIncomingEvent>>(`stone.useReact.errorHandlers.${String(error?.name ?? 'default')}`)

  let layoutInstance: (IComponentEventHandler<ReactIncomingEvent> | undefined)

  if (isMetaClassModule<ComponentEventHandlerClass<ReactIncomingEvent>>(metavalue)) {
    layoutInstance = new metavalue.module.prototype.constructor(blueprint)
  } else if (isMetaFactoryModule<FactoryComponentEventHandler<ReactIncomingEvent>>(metavalue)) {
    layoutInstance = metavalue.module(blueprint)
  }

  const componentType = (layoutInstance?.render.bind(layoutInstance) as (ElementType | undefined)) ?? StoneErrorComponent
